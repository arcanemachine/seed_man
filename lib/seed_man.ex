defmodule SeedMan do
  ...
  @seed_files_directory_path "priv/repo/seeds"
  ...
  def dump_schema_to_seed_file(repo_module, schema_module, opts \\ []) do
    ...
    seed_file_path = get_seed_file_path(table_name)
  ...
  @doc false
  def eval_seed_data_from_seed_file(schema_module) do
    schema_module
    |> read_seed_file()
    |> Code.eval_string()
    |> elem(0)
  ...
  @doc """
  Load table data for a `schema_module` from an existing seed data file (to save seed data, see
  `dump_schema_to_seed_file/3`).

  This function is intended to make it easy to load seed data that was previously saved to
  `#{@seed_files_directory_path}` to make it easier to work with a local database during
  development.

  > #### Tip {: .tip}
  >
  > Before calling this function, ensure that you have started the server with the application
  > configured to use the correct database you want to load the seed data from.
  ...
  def load_schema_from_seed_file(repo_module, schema_module) do
    table_name = schema_module.__schema__(:source)
    seed_file_path = get_seed_file_path(table_name)

    seed_data = eval_seed_data_from_seed_file(schema_module)

    Logger.info(~s(Loading seed data for the table "#{table_name}" from "#{seed_file_path}"...))
    repo_module.insert_all_with_autogenerated_timestamps(schema_module, seed_data)
  ...
  """
  def read_seed_file(schema_module, opts \\ []) do
  ...
  defp get_seed_file_path(table_name) do
    Application.app_dir(
      :resort_indexer,
